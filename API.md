...
仅允许软删除非管理员账号。软删除后该用户不再出现在默认用户列表，也不能再通过 token 登录或鉴权。
...
管理后台默认只显示未删除用户；已删除用户可在 `/admin/deleted-users` 查看并恢复。
...
软删除指定用户。删除后该用户不会出现在 `/admin` 和 `GET /api/users`，可在 `/admin/deleted-users` 恢复。
...
**成功响应**：`303` 跳转到 `/admin/deleted-users`
